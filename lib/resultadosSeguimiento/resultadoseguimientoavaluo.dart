...
import 'dart:convert';
import 'package:insejupy_multiplatform/detalleseguimiento/detalleseguimientoavaluo.dart';
import 'package:insejupy_multiplatform/constants.dart';

import '../seguimiento.dart';

class ResultadosSeguimientoAvaluo extends StatelessWidget {
  final String informacion;
  final String iIDSolicitud;
  final int iIDAnioFiscal;

  const ResultadosSeguimientoAvaluo({
    Key? key,
          ...
          if (index == 0) {
            ...
            return Card(
              ...
              child: Column(
                ...
                children: [
                  ListTile(
                    title: const Text('Trámite: Avaluo'),
                    subtitle: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Container(),
                        Text(
                          'Etapa: ${_getEtapaText(resultado['cEstatus'])}',
                          style: const TextStyle(
                            fontSize: 16,
                          ),
                        ),
                        Text(
                          'Fecha: ${resultado['cFechaSeguimiento']}',
                          style: const TextStyle(
                            fontSize: 16,
                          ),
                        ),
  ...
  Future<void> obtenerDetallesDeAPI(
    ...
    try {
      ...
      String apiUrl = '$baseUrl/avaluo/lineaTiempo';
      ...
      if (response.statusCode == 200) {
        ...
        await Navigator.push(
          context,
          MaterialPageRoute(
            builder: (context) => DetallesSeguimientoAvaluo(detalles: data),
          ),
        );
      } else {
        if (kDebugMode) {
  ...
  String _getEtapaText(String cEstatus) {
    List<String> etapasEnProceso = [
      "Revision",
      "Revision (subsanado)",
      "Visto Bueno Jefe",
      "Recálculo",
    ...
    if (etapasEnProceso.contains(cEstatus)) {
      return "En proceso";
    } else {
      return cEstatus;
    }
